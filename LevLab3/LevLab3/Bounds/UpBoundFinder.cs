...
{
    internal class UpBoundFinder : IBoundFinder
    {
        private DataFromOrders _orders;
        public UpBoundFinder(DataFromOrders orders) 
        {
            _orders = orders;
        }
        ...
        private int GetNextNode(int lastNode, int currentTime, List<int> possibleNodes)
        {
            ...
            {
                return nodesWithoutFine.OrderBy(index => _orders._times[lastNode, index]).First();
            }
